...
| **Keyless/OIDC + ssh transports for CI/agents** — decrypt with a per-context identity, never the passphrase | "Agent pulling secrets in CI or over ssh, safely" | **XL** | ✅ **Env-key transport landed:** `TVAULT_IDENTITY_KEY` (a `tvault-key1…` string) carries a per-context identity with no file/passphrase; one resolver (`resolveIdentity`, file-beats-env-key + warn) backs `open`/`decrypt-env`/`env --identity`/git filters, so the same key decrypts in CI, over ssh (`seal \| ssh host 'tvault open'`), and for agents. `tvault identity export` provisions it (TTY-guarded), `tvault ci init --mode=identity` scaffolds a passphrase-free workflow. **Still deferred:** the full **OIDC keyless** flow (bind a CI OIDC token → recipient automatically) — needs an OIDC verifier + provider trust config + likely a new dependency; route it through `resolveIdentity` when added. |
