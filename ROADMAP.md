...
| Item | Why | Effort | Critical refinement |
|---|---|---|---|
| **Asymmetric recipient layer** — `internal/crypto/recipient.go`: X25519 → HKDF-SHA256 → ChaCha20-Poly1305 wrapping of the project DEK; `tvault identity new/list`, `tvault project share/unshare --recipient tvault1…` | Relocates trust from "everyone shares the passphrase" to "per-actor keypairs" — *genuinely safe* sharing | **XL** | ✅ **Landed (foundation):** `recipient.go` (WrapDEK/UnwrapDEK + Identity, crypto/ecdh + chacha20poly1305 + hkdf, tvault-native stanza, **no new dependency**) and `tvault identity new/list` are built, rigorously tested, and crypto-reviewed. **Still to do:** `project share/unshare` with a `[]DEKWrap` stored beside the KEK wrap (backward-compatible). **Two must-dos when wiring share:** (1) age/sops interop stays deferred. (2) **revocation ≠ re-wrap** — a revoked holder already has the DEK, so real `unshare` must mint a fresh DEK and **re-encrypt every value in the project**, then re-wrap for the rest; audit grants/revokes. |
