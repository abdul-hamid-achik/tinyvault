...
| **TUI: live `.env` drift indicator + value diff** | Answers "is my `.env` in sync with the vault?" at a glance | **L** | Read-only; pairs with `sync`. |
| **Secret history & rollback in the CLI** (`tvault history KEY`, `tvault rollback KEY`) | The `Version` field already exists on `SecretEntry` | **L** | Surfacing existing data; storage already versions on overwrite. |
...
| Item | Why | Effort | Critical refinement |
|---|---|---|---|
| **Asymmetric recipient layer** — `internal/crypto/recipient.go`: X25519 → HKDF-SHA256 → ChaCha20-Poly1305 wrapping of the project DEK; `tvault identity new/list`, `tvault project share/unshare --recipient tvault1…` | Relocates trust from "everyone shares the passphrase" to "per-actor keypairs" — *genuinely safe* sharing | **XL** | All primitives are already in vendored `golang.org/x/crypto` — **no new dependency**. Store a `[]DEKWrap` alongside the existing KEK wrap (backward-compatible). **Two must-dos:** (1) **defer age/sops interop** — `filippo.io/age` isn't vendored; ship a `tvault`-native stanza first. (2) **revocation ≠ re-wrap.** A revoked holder already has the DEK; real `unshare` must mint a fresh DEK and **re-encrypt every value in the project**, then re-wrap for remaining recipients — otherwise it's security theater. Audit grants/revokes. |
| **`.env.encrypted` v2** — commit-safe, per-recipient-wrapped DEK, **no dependence on the local KEK** | "Commit secrets to the repo, they decode when needed" (the dotenvx/SOPS pattern) | **L** | Bump the format `version` byte (already reserved). `decrypt-env` auto-detects v1 (KEK) vs v2 (identity). Depends on `recipient.go`. |
| **Git clean/smudge filters** — `tvault git-filter` to auto-encrypt on commit / decrypt on checkout | "Have them decode themselves" with zero manual steps | **L** | ⚠️ Standalone (without recipients) this is just git-crypt-with-one-shared-passphrase. **Hard-depends on Spine A.** Document the bootstrapping reality: a teammate/CI still needs their *private* key delivered once. |
