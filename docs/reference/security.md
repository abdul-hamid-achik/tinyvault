...
- **An AI agent accidentally pulls a secret into its prompt or logs.** The MCP layer offers workflows that do not intentionally return values: `vault_run_with_secrets` injects selected values into a subprocess, `vault_export_env` and `vault_open_sealed` write to disk and return a path, `vault_generate_secret` returns generation metadata but not the generated value, and `vault_seal_for_recipients` returns ciphertext. `vault_get_secret` deliberately returns a raw value, `vault_set_secret` accepts one from the client, and subprocesses can leak or transform what they receive; policy is the real control.
...
| `vault_open_sealed` | a file path on disk, not the contents; requires write access |
...
Get, set, delete, project create/delete, rollback, history, and bulk project reads (`GetAllSecrets` / `GetSelectedSecrets`, including `tvault env` and `tvault run`) write the audit log in the vault layer, so CLI, MCP, and the agent share one trail. Bulk reads are one project-level `secret.read` (count only, never values). Surface handlers still log generate, exec, export, share, open-sealed, and env-group mutations. The trail is not exhaustive: unlocking and passphrase rotation are not recorded. Query it with `tvault audit` or MCP `vault_audit_log`. `tvault ssh` injects selected values into a remote process over the SSH channel without writing a remote file; those values are still plaintext in that process's environment, the same residual risk as `tvault run`. `tvault docker build` feeds BuildKit secret mounts (tmpfs in the build); `docker compose` and `docker run` still place plaintext in a process or container environment. `tvault mcp --connect auto` can serve secret reads through the local agent so a GUI host need not store `TVAULT_PASSPHRASE`; that does not widen the agent's same-uid trust boundary, and writes still need the passphrase.
...
::: info Generation never returns the value
`tvault generate` and MCP `vault_generate_secret` store a random secret and return only metadata. The audit log is read with `tvault audit` and over MCP (`vault_audit_log` / `vault_audit_log_since`).
:::
