...
var (
	...
	// another tvault process holds bbolt's exclusive lock. Distinct from
	// ErrLocked, which means the vault exists but is locked at rest.
	ErrVaultBusy = errors.New("vault is locked by another tvault process " +
		"(the database is open — e.g. a long-running 'tvault run'). " +
		"Find it with: pgrep -fl tvault")
