	...
	return docsCatalog{
		Features: []docsFeature{
			...
			},
			{
				...
				Description: "`tvault status --json` is lock-free and reports {initialized, locked, agent_running, agent_accessible, vault_dir, current_project, project_count}. `agent_running` means a local socket is present; `agent_accessible` means this process can use it for the selected project (explicit `--project`, then current, then `default`) with its current TVAULT_AGENT_TOKEN. `locked` is false only when a value read is actually available without a passphrase, so a token-required agent does not create a false 'unlocked' report. When an unlock-requiring command runs non-interactively (stdin not a TTY) with no TVAULT_PASSPHRASE and no usable agent, it fails fast with exit code 3 and, under --json, prints {\"error\":\"vault_locked\",\"locked\":true} on stdout with nothing on stderr — instead of prompting or emitting an opaque read error.",
			},
			{
			...
			},
			{
				...
				Description: "Two layers, both keyed by the recipient layer so no passphrase ever touches the files. (1) Standalone: `encrypt-env --recipient` writes a self-contained v2 .env.encrypted that any holder of a matching identity opens with `decrypt-env --identity` — KEK-independent, so passphrase rotation does not invalidate it. (2) Transparent git filters: `git-filter install` registers clean/smudge filters and `git-filter track <pattern>` adds .gitattributes entries, so matched files are stored encrypted in history but appear as plaintext in the working tree for anyone holding a recipient identity. Recipients live in a committed .tvault-recipients file (public keys only) so the read-set travels with the repo. Without an identity, files stay 'locked' (ciphertext) rather than failing checkout; the clean filter re-emits unchanged blobs so git status stays quiet; identity resolution is $TVAULT_IDENTITY, then `git config tvault.identity`, then 'default'. Over MCP, an agent can produce the same v2 blob with vault_seal_for_recipients (ciphertext only) and open it with vault_open_sealed, which writes a 0600 dotenv and returns only the path and key names. For CI / ssh / agents, supply a per-context identity via the TVAULT_IDENTITY_KEY environment variable (a tvault-key1… string) so decrypt-env / open / git-filter decrypt with NO passphrase and no key file; a local identity file takes precedence over the env key (and tvault warns when it does). Provision it with `tvault identity export <name> --force | gh secret set TVAULT_IDENTITY_KEY`, and scaffold a workflow with `tvault ci init --mode=identity`.",
			},
			{
				Name:        "diagnostics",
